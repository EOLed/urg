...
class SecuredActionsController extends UrgAppController {
	...
	function view($id = null) {
		if (!$id) {
			$this->Session->setFlash(__('Invalid secured action', true));
	...
	function add() {
		if (!empty($this->data)) {
			...
			if ($this->SecuredAction->save($this->data)) {
				$this->Session->setFlash(__('The secured action has been saved', true));
				$this->redirect(array('action' => 'index'));
			} else {
				$this->Session->setFlash(
                        __('The secured action could not be saved. Please, try again.', true));
			}
		}
	...
	function delete($id = null) {
		if (!$id) {
			$this->Session->setFlash(__('Invalid id for secured action', true));
			$this->redirect(array('action'=>'index'));
		}
		if ($this->SecuredAction->delete($id)) {
			$this->Session->setFlash(__('Secured action deleted', true));
			$this->redirect(array('action'=>'index'));
		}
		$this->Session->setFlash(__('Secured action was not deleted', true));
		$this->redirect(array('action' => 'index'));
	}
